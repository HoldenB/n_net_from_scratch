...
def ex_two() -> None:
    ...
    loss = -reduce(
        lambda a, b: a + b,
        *[map(lambda tup: math.log(tup[0]) * tup[1], zip(softmax_output, target_output_one_hot_vector))]
    )
    print(loss)
    print()
    
    # We can do this easily with numpy      # According to our class_targets:
    softmax_outputs = [[0.7, 0.2, 0.1],     # index 0 for dog = 0.7
